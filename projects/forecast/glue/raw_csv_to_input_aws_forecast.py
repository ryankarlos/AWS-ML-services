...
ts_dyf = glueContext.create_dynamic_frame.from_catalog(database="default", table_name="sample_manning_csv",
...
filtered_df = dyf_MapItemid.toDF().filter(
    (year(col("timestamp")) > filter_years[0]) & (year(col("timestamp")) < filter_years[1]))
